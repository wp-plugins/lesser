 ...
 */
abstract class LesserAdminPageFramework_Factory_Controller extends LesserAdminPageFramework_Factory_View {
    ...
    public function setFieldErrors($aErrors) {
        $GLOBALS['aLesserAdminPageFramework']['aFieldErrors'] = $this->oUtil->getElement($GLOBALS, array('aLesserAdminPageFramework', 'aFieldErrors'), array());
        if (empty($GLOBALS['aLesserAdminPageFramework']['aFieldErrors'])) {
            add_action('shutdown', array($this, '_replyToSaveFieldErrors'));
        }
        $_sID = md5($this->oProp->sClassName);
        $GLOBALS['aLesserAdminPageFramework']['aFieldErrors'][$_sID] = isset($GLOBALS['aLesserAdminPageFramework']['aFieldErrors'][$_sID]) ? $this->oUtil->uniteArrays($GLOBALS['aLesserAdminPageFramework']['aFieldErrors'][$_sID], $aErrors) : $aErrors;
    }
    public function hasFieldError() {
        return isset($GLOBALS['aLesserAdminPageFramework']['aFieldErrors'][md5($this->oProp->sClassName) ]);
    }
    public function setSettingNotice($sMessage, $sType = 'error', $asAttributes = array(), $bOverride = true) {
        $GLOBALS['aLesserAdminPageFramework']['aNotices'] = $this->oUtil->getElement($GLOBALS, array('aLesserAdminPageFramework', 'aNotices'), array());
        if (empty($GLOBALS['aLesserAdminPageFramework']['aNotices'])) {
            add_action('shutdown', array($this, '_replyToSaveNotices'));
        }
        $_sID = md5(trim($sMessage));
        if ($bOverride || !isset($GLOBALS['aLesserAdminPageFramework']['aNotices'][$_sID])) {
            ...
            $GLOBALS['aLesserAdminPageFramework']['aNotices'][$_sID] = array('sMessage' => $sMessage, 'aAttributes' => $_aAttributes + array('class' => $sType, 'id' => $this->oProp->sClassName . '_' . $_sID,),);
        }
    }
    public function hasSettingNotice($sType = '') {
        $_aNotices = $this->oUtil->getElementAsArray($GLOBALS, array('aLesserAdminPageFramework', 'aNotices'), array());
        if (!$sType) {
